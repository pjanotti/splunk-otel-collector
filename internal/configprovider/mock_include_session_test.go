...
func (is *includeSession) Retrieve(_ context.Context, selector string, params interface{}) (configsource.Retrieved, error) {
	...
	if !is.WatchFiles {
		return NewRetrieved(buf.Bytes(), WatcherNotSupported), nil
	}
	...
	return NewRetrieved(buf.Bytes(), watchForUpdateFn), nil
}
...
func newIncludeSession(config Config) *includeSession {
